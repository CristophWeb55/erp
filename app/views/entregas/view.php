    ...
    <div style="flex: 1.4; display: flex; flex-direction: column; gap: 20px;">
        ...
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px;">
            ...
            <div class="glass-panel"
                style="padding: 15px; border-radius: 16px; border-left: 3px solid var(--accent-color);">
                <h4
    ...
    <div style="flex: 1;">
        <div class="glass-panel" style="padding: 20px; border-radius: 20px; position: sticky; top: 10px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
                <h3
                    style="font-weight: 800; font-size: 16px; color: var(--text-primary); margin: 0; display: flex; align-items: center; gap: 8px;">
                    <i class="fas fa-file-signature" style="color: var(--accent-secondary);"></i> Finalizar Entrega
                </h3>
                <?php if ($entrega['estatus'] != 'Entregado'): ?>
                    <button onclick="document.getElementById('modalProgramar').style.display='flex'"
                        style="background: none; border: none; color: var(--accent-color); font-size: 11px; font-weight: 700; cursor: pointer; text-decoration: underline;">
                        <i class="fas fa-edit"></i> Programar Despacho
                    </button>
                <?php endif; ?>
            </div>
            ...
            <?php if (isset($_GET['msg']) && $_GET['msg'] == 'updated'): ?>
                <div
                    style="background: rgba(16, 185, 129, 0.1); color: #10b981; padding: 10px; border-radius: 8px; font-size: 11px; font-weight: 700; margin-bottom: 15px;">
                    ✓ Datos de despacho actualizados.
                </div>
            <?php endif; ?>
            ...
            <?php else: ?>
                ...
                <form id="formConfirm" action="index.php?controller=Logistica&action=confirm" method="POST"
                    ...
                    <!-- Campos de despacho ocultos o pre-llenados -->
                    <input type="hidden" name="placas_vehiculo" value="<?= $entrega['placas_vehiculo'] ?? '' ?>">
                    <input type="hidden" name="guia_seguimiento" value="<?= $entrega['guia_seguimiento'] ?? '' ?>">
                    <input type="hidden" name="bultos" value="<?= $entrega['bultos'] ?? 1 ?>">
                    <input type="hidden" name="peso_total" value="<?= $entrega['peso_total'] ?? 0 ?>">
                    ...
                    <div style="background: rgba(41, 56, 135, 0.04); padding: 12px; border-radius: 12px;">
                        <span
                            ...
                            de Recepción</span>
                        <div style="display: flex; flex-direction: column; gap: 8px;">
                            <input type="text" name="persona_recibe" placeholder="¿Quién recibe?"
                                style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #ddd; font-size: 12px;">
                            <input type="text" name="telefono_contacto" placeholder="Teléfono de contacto"
                                style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #ddd; font-size: 12px;">
                        </div>
                    </div>
...
<!-- Modal para Programar Despacho -->
<div id="modalProgramar"
    style="display: none; position: fixed; inset: 0; background: rgba(0,0,0,0.5); backdrop-filter: blur(5px); z-index: 1000; align-items: center; justify-content: center; padding: 20px;">
    <div class="glass-panel"
        style="background: white; width: 100%; max-width: 450px; border-radius: 24px; padding: 25px; animation: slideUp 0.3s ease-out;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
            <h3 style="margin: 0; font-weight: 800; color: var(--text-primary);">Programar Despacho</h3>
            <button onclick="document.getElementById('modalProgramar').style.display='none'"
                style="background: none; border: none; color: var(--text-secondary); cursor: pointer;"><i
                    class="fas fa-times"></i></button>
        </div>

        <form action="index.php?controller=Logistica&action=updateLogistics" method="POST"
            style="display: flex; flex-direction: column; gap: 15px;">
            <input type="hidden" name="id" value="<?= $entrega['id'] ?>">

            <div>
                <label
                    style="display: block; font-size: 10px; font-weight: 800; color: var(--text-secondary); text-transform: uppercase; margin-bottom: 5px;">Placas
                    del Vehículo</label>
                <input type="text" name="placas_vehiculo" value="<?= $entrega['placas_vehiculo'] ?? '' ?>"
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                <div>
                    <label
                        style="display: block; font-size: 10px; font-weight: 800; color: var(--text-secondary); text-transform: uppercase; margin-bottom: 5px;">Bultos</label>
                    <input type="number" name="bultos" value="<?= $entrega['bultos'] ?? 1 ?>"
                        style="width: 100%; padding: 10px; border-radius: 10px; border: 1px solid #e2e8f0;">
                </div>
                <div>
                    <label
                        style="display: block; font-size: 10px; font-weight: 800; color: var(--text-secondary); text-transform: uppercase; margin-bottom: 5px;">Peso
                        Total (KG)</label>
                    <input type="number" step="0.01" name="peso_total" value="<?= $entrega['peso_total'] ?? 0 ?>"
                        style="width: 100%; padding: 10px; border-radius: 10px; border: 1px solid #e2e8f0;">
                </div>
            </div>
            ...
            <div>
                <label
                    style="display: block; font-size: 10px; font-weight: 800; color: var(--text-secondary); text-transform: uppercase; margin-bottom: 5px;">Cambiar
                    Estatus</label>
                <select name="estatus"
                    style="width: 100%; padding: 10px; border-radius: 10px; border: 1px solid #e2e8f0;">
                    <option value="Programado" <?= $entrega['estatus'] == 'Programado' ? 'selected' : '' ?>>Programado
                    </option>
                    <option value="En Tránsito" <?= $entrega['estatus'] == 'En Tránsito' ? 'selected' : '' ?>>En Tránsito
                    </option>
                    <option value="Incidencia" <?= $entrega['estatus'] == 'Incidencia' ? 'selected' : '' ?>>Incidencia
                    </option>
                </select>
            </div>

            <button type="submit" class="btn"
                style="background: var(--accent-color); color: white; border: none; padding: 12px; border-radius: 12px; font-weight: 800; margin-top: 10px;">
                GUARDAR CAMBIOS
            </button>
        </form>
    </div>
