...
try {
    ...
    $check = $db->query("SHOW COLUMNS FROM terceros LIKE 'activo'");
    if ($check->rowCount() == 0) {
        $sql = "ALTER TABLE terceros ADD COLUMN activo BOOLEAN DEFAULT TRUE";
        $db->query($sql);
        echo "Columna 'activo' agregada correctamente a la tabla 'terceros'.\n";
    } else {
